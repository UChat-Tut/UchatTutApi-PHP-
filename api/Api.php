...
abstract class Api
{
    ...
    public function __construct()
    {
        ...
        $this->requestUri[count($this->requestUri) - 1] = explode('?', $this->requestUri[count($this->requestUri) - 1])[0];
    ...
    protected function responseError($errorCode = 100, $status = 405)
    {
        header("HTTP/1.1 " . $status . " " . $this->requestStatus($status));
        ...
        $data = [
            'error' => [
                'code' => $errorCode,
                'msg' => $this->requestError($errorCode)
            ]
        ];
        return json_encode($data);
    }
    ...
    private function requestError($errorCode)
    {
        ...
        return ($status[$errorCode]) ? $status[$errorCode] : $status[100];
    }
